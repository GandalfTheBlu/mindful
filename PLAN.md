...
- **Voice waveform not visible for briefing and learning messages** — the waveform canvas visualisation stops appearing for messages generated by the Briefing and Learning buttons. Regression that has recurred before; needs investigation in the TTS/canvas initialisation path for non-chat-originated messages.

- **Waveform appears on the previous message** — when a waveform does render, it attaches to the preceding message bubble rather than the one currently being spoken. Canvas or AudioContext is being bound to the wrong DOM element.

- **Learning proposal message does not always get a TTS stream** — the initial topic-proposal message streamed by the Learning button is intermittently silent. Likely a timing issue: TTS may be initialised before the bubble is ready, or the StreamingTTS instance is being created against the wrong element.

- **Deep research sub-step visibility in UI** — tool calls that run multi-step sub-processes (primarily `deep_research`, which runs a full webResearch loop internally) show no granular progress. The briefing UI already solves this with a per-task status map; the same pattern should be applied to tool-call status events so the user can see individual search and fetch steps as they happen inside a `deep_research` call.
